...
英文と音声で覚える「学習」と、日本語から英語を思い出す「3秒チャレンジ」を切り替えられる自己学習用のWebアプリです。

公開先: **https://ryuya-dot-com.github.io/Daily-English/**

## 使い方

1. 練習モード、section、出題順（掲載順／ランダム）を選びます。
2. 「練習を始める」を押すと、音声の準備後に練習が始まります。
3. 「もう一度聞く」または「次へ」で進めます。「自動で次へ」を有効にすると、音声終了の2秒後に次の文へ進みます。

| モード | 練習の流れ |
| --- | --- |
| 学習 | 日本語と英文を最初から表示し、模範音声を再生します。時間制限はありません。英文を見ながら、繰り返し聞いて声に出せます。 |
| 3秒チャレンジ | 日本語の表示と同時に3秒のタイマーが始まり、バーがなくなると英文を表示して模範音声を再生します。 |

初めての文は「学習」で確認してから、同じsectionを「3秒チャレンジ」で練習できます。初期選択は従来どおり「3秒チャレンジ」です。モードを変更するときは「場面を選び直す」で選択画面に戻ります。
...
JavaScriptの検証はNode.jsの標準機能で時計・音声・DOMを模擬し、学習モードの即時表示とタイマーなしの動作、モード切替、3秒前の答え非表示、一時停止、音声終了後の自動送り、再生し直し、読み込み失敗時の再試行、画面切替後に届く古い応答、セッション完了を確認します。実機のブラウザ・音声出力の確認は別途必要です。
